...
const main = async () => {
  ...
  const saveStates = async (agent: Talos, close: boolean = false) => {
    if (!statesFile) {
      return;
    }
    agent.gatherStates();
    await statesFile.truncate(0);
    await statesFile.write(yaml.stringify(agent.states), 0, "utf8");
    if (close) {
      await statesFile.close();
    }
    logger.info("States file is saved");
    logger.info(`States: ${JSON.stringify(agent.states)}`);
  };
  ...
  const agent = new Talos(config, states);
  ...
  process.on("SIGUSR1", async () => {
    ...
    await saveStates(agent);
    await agent.unloadPlugins();
    await agent.loadPlugins();
    logger.info("Talos is reloaded");
    reloading = false;
  });
