...
export default class Shell extends PluginBase {
      ...
      },
      {
        fn: async (args: Dict<any>) => {
          ...
          process.on("close", (code) => {
            ...
            agent.emitEvent("shell/terminated", {
              pid: process.pid,
              code,
  ...
  async unload(agent: Talos) {
    agent.deregisterTool("shell/exec");
    agent.deregisterTool("shell/write-stdin");
    agent.deregisterTool("shell/kill");
    agent.deregisterTool("shell/apt-install");
    agent.deregisterEvent("shell/stdout");
    agent.deregisterEvent("shell/terminated");
  }
}
