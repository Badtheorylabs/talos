...
export default class Discord extends PluginBase {
  ...
  async load(agent: Talos) {
    ...
    agent.emitPrivateEvent("discord/load", {
    ...
    agent.once("plugins-loaded", () => {
      this.client.on(Events.MessageCreate, async (message) => {
        ...
        if (!this.config.allowed_channel_ids.includes(message.channel.id)) {
          if (
            ...
            message.channel.send(
              `You appear to not have access to Talos, but FYI, your channel ID is ${message.channel.id}.`,
            );
          }
          return;
        }
        ...
        agent.emitEvent("discord/message-received", {
          id: message.id,
          author: {
      ...
      this.client.on(Events.InteractionCreate, async (interaction) => {
        ...
        agent.emitEvent("discord/interaction-received", {
          id: interaction.id,
          user: {
  ...
  async unload(agent: Talos) {
    agent.emitPrivateEvent("discord/unload", {
      content: "Plugin discord unloaded.",
    });
    agent.off("private-event", this.boundAgentPrivateEventHandler);
    await this.client.destroy();
    agent.deregisterTool("discord/send-message");
    agent.deregisterTool("discord/edit-message");
    agent.deregisterTool("discord/delete-message");
    agent.deregisterEvent("discord/message-received");
    agent.deregisterEvent("discord/interaction-received");
  }
