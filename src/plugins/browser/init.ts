...
export default class Browser extends PluginBase {
  ...
  async load(agent: Talos) {
    this.agent = agent;
    agent.registerNetwork("browser", {
      name: "Browser",
      endpoints: ["user-provided URLs"],
      sendsUserData: true,
    });
  ...
  async popupHandler(fromIndex: number, index: number) {
    const metadata = await this.browserUse.getPageMetadata(index);
    this.agent.emitEvent("browser/popup", {
  ...
  async downloadStartedHandler(pageIndex: number, filename: string) {
    this.agent.emitEvent("browser/download-started", {
  ...
  async downloadCompletedHandler(pageIndex: number, filename: string) {
    this.agent.emitEvent("browser/download-completed", {
      page_index: pageIndex,
      filename,
    });
  }
  ...
  async unload(agent: Talos) {
    ...
    agent.deregisterTool("browser/new-page");
    agent.deregisterTool("browser/click");
    agent.deregisterTool("browser/fill");
    agent.deregisterTool("browser/get-content");
    agent.deregisterTool("browser/get-element-data");
    agent.deregisterTool("browser/screenshot");
    agent.deregisterTool("browser/scroll-down");
  }
}
