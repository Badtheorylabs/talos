...
export default class Telegram extends PluginBase {
  ...
  async load(agent: Talos) {
    agent.registerNetwork("telegram", {
      name: "Telegram",
    ...
    agent.emitPrivateEvent("telegram/load", {
    ...
    agent.once("plugins-loaded", () => {
      this.bot.on("message", async (msg) => {
        ...
        if (!this.config.allowed_chat_ids.includes(chatId)) {
          if (
            ...
            this.bot.sendMessage(
              chatId,
              `You appear to not have access to Talos, but FYI, your chat ID is ${chatId}.`,
            );
          }
          return;
        }
        ...
        agent.emitEvent("telegram/message-received", {
      ...
      this.bot.on("callback_query", async (query) => {
        agent.emitEvent("telegram/callback-query-received", {
          id: query.id,
          from: {
            id: query.from.id,
  ...
  async unload(agent: Talos) {
    agent.emitPrivateEvent("telegram/unload", {
      content: "Plugin telegram unloaded.",
    });
    agent.off("private-event", this.boundAgentPrivateEventHandler);
    await this.bot.stopPolling();
    agent.deregisterTool("telegram/send-message");
    agent.deregisterTool("telegram/edit-message");
    agent.deregisterTool("telegram/delete-message");
    agent.deregisterEvent("telegram/message-received");
    agent.deregisterEvent("telegram/callback-query-received");
  }
  ...
  agentPrivateEventHandler(event: string, args: Dict<any>) {
    ...
    if (
      ["cerebrum/thinking", "talos/tool-call", "talos/tool-result"].includes(
        event,
      )
    ) {
      const message = (() => {
