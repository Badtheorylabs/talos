...
export default class Llm extends PluginBase {
      ...
      },
      {
        fn: async (args) => {
          agent.recordModelCall(
            args.model,
            this.config.base_url,
            `llm/generate-image called ${args.model}.`,
          );
  ...
  async unload(agent: Talos) {
    ...
    agent.deregisterTool("llm/chat");
    agent.deregisterTool("llm/generate-image");
  }
