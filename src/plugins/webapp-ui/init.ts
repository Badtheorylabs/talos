...
export default class WebappUI extends PluginBase {
  ...
  async load(agent: Talos) {
    this.agent = agent;
    agent.registerNetwork("webapp-ui", {
      name: "Supabase web app UI",
      endpoints: [this.config.supabase.url],
      sendsUserData: true,
    });
    ...
    this.supabase.auth.onAuthStateChange((event, session) => {
      if (event === "SIGNED_IN" || event === "TOKEN_REFRESHED") {
        ...
        this.agent.emitPrivateEvent("webapp-ui/token-refreshed", {
    ...
    if (error2) {
      ...
      this.agent.states = data[0].states;
      this.logger.info(
  ...
  async unload(agent: Talos) {
    ...
    this.agent.gatherStates();
    const { data, error } = await this.supabase
      .from("contexts")
      .update({
        states: this.agent.states,
      })
      .eq("id", this.config.context_id)
      .select();
    if (error) {
